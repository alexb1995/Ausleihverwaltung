...
/*PAGE SETZEN*/
$PAGE->set_url('/mod/checkdeadline/edit.php', array('id' => $cm->id,'resourceid' => $_GET['resourceid']));
...
if(strpos($strUrl, 'resourceid=')==true){
    ...
    $sql= 'SELECT * FROM {checkdeadline_resources} WHERE id ='.$resID.';';
    ...
    if ($fromform = $mform->get_data()) {
        ...
        $process_definition_id = checkdeadline_get_process_definition_id("meisterkey"); //key aus dem Prozessmodel
        //error_log("PROCESS DEFINITION ID IS: " . $process_definition_id);
        $process_instance_id = checkdeadline_start_process($process_definition_id, 'businesskey');
        ...
        */
        ...
        /*Activit*/
        //$result = checkdeadline_answer_input_required_resources($taskid, $process_definition_id, $fm_name, $fm_description, $fm_serialnumber, $fm_inventorynumber,$fm_comment,$fm_status,$fm_amount,$fm_type,$fm_maincategory,$fm_subcategory);
    ...
    if ($fromform = $mform->get_data()) {
        ...
        */
        ...
        $DB->update_record('checkdeadline_resources',$record,$bulk=false); 
